...
int main()
{
    ...
    glGenBuffers(1, &VBO);     // Create VBO ID

    glBindVertexArray(VAO);    // Bind VAO with current context

    glBindBuffer(GL_ARRAY_BUFFER, VBO);// Bind VBO with VAO
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);// Copy Vertex data from CPU to GPU (VBO)

    // attribute location = 0, component per vertex = 3, Data type, Normalization, Stride, Offset
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Find the location of the "transform" uniform in the shader
        unsigned int transformLoc = glGetUniformLocation(shaderProgram, "transform");
        // Upload the modelMatrix data from CPU to the shader uniform(loc, number of matrix, transpose, pointer to the model)
